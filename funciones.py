...
def eliminar_activo(matriz, ticker):
    ...
    while ticker.upper() != 'FIN':
        ...
        if activos != False:
            if len(activos) > 1:
                menu_activos_repetidos(activos)
    ...
    while nombre.upper() != 'FIN':
        ...
        if activo_encontrado:
            ...
            if len(activo_encontrado) > 1:
                menu_activos_repetidos(activo_encontrado)

                opcion = input("\033[93mEscoja el nombre a eliminar por su metodologia:\033[0m ")
    ...
    while validar_metodologia(met_op):
        ...
        if len(coincidencias_parciales) == 1:
            ...
            if confirmacion == 'n':
                menu_metodologia(metodos_validos)
                met_op = input('\033[93mIngrese la metodología de operación (min. 3 letras):\033[0m ').replace(' ', '').lower()
            elif int(confirmacion) <= len(coincidencias_parciales) and int(confirmacion) > 0:
...
def busqueda_ticker(matriz, ticker):
    ...
    if len(tickers_encontrados) == 1:
        ...
        print("\033[31mActivo no encontrado. Asegúrese que tenga 0 unidades en tesorería.\033[0m")
        return False
...
def busqueda_nombre(matriz, nombre):
    ...
    if len(nombres_encontrados) == 1:
        ...
        print("\033[31mActivo no encontrado.\033[0m")
        return False
...
def validar_nombre(nombre):
    ...
    if nombre == '' or nombre.isnumeric():
        print('\033[31mEl nombre del activo no puede estar vacío ni contener caracteres no alfabéticos. Intente nuevamente.\033[0m')
        return False
    else:
        return True
...
def validar_ticker(ticker):
    ...
    if ticker == '' or ticker.isnumeric() or len(ticker) < 3 or len(ticker) > 5 or ticker.upper() == 'FIN':
        print('\033[31mEl ticker debe tener entre 3 y 5 caracteres, no puede estar vacío, y solo puede contener caracteres alfabéticos. Intente nuevamente.\033[0m')
        return False
    else:
        return True
...
def validar_valor(valor_ref):
    ...
    if valor_ref == '' or valor_ref.count(".") > 1 or not valor_ref.replace(".", "").isnumeric() or float(valor_ref) <= 0:
        print('\033[31mEl valor de referencia debe ser un número positivo, y no puede estar vacío. Intente nuevamente.\033[0m')
        return False
    else:
        return True
...
def validar_volumen(volumen):
    ...
    if volumen == '' or not volumen.isnumeric() or int(volumen) < 0 :
        print('\033[31mEl volumen de actividad debe ser un número positivo, y no puede estar vacío. Intente nuevamente.\033[0m')
        return False
    else: 
        return True
...
def validar_unidades(unidades):
    ...
    if unidades == '' or unidades.count(".") > 1 or not unidades.replace(".", "").isnumeric() or float(unidades) < 0 :
        print('\033[31mLas unidades totales en tesorería deben ser un número positivo, y no pueden estar vacías. Intente nuevamente.\033[0m')
        return False
    else:
        return True
...
def validar_puntaje(puntaje):
    ...
    if puntaje == '' or not puntaje.isnumeric() or int(puntaje) < 1 or int(puntaje) > 10 :
        print('\033[31mEl puntaje de confianza debe ser un número entre 1 y 10, y no puede estar vacío. Intente nuevamente.\033[0m')
        return False
    else:
        return True
...
def validar_opcion_modificar(opcion):
    ...
    if opcion == '' or not opcion.isnumeric() or int(opcion) < 1 or int(opcion) > 8:
        print("\033[31mOpción inválida. Intente nuevamente.\033[0m")
        return False
    else: 
        return True
...
def menu_modificar():
    ...
    print("\033[32m1. Cambiar nombre del activo\033[0m")
    print("\033[32m2. Cambiar ticker\033[0m")
    print("\033[32m3. Cambiar valor de referencia\033[0m")
    print("\033[32m4. Cambiar volumen de actividad\033[0m")
    print("\033[32m5. Cambiar metodología de operación\033[0m")
    print("\033[32m6. Cambiar unidades en tesorería\033[0m")
    print("\033[32m7. Cambiar puntaje de confianza (1-10)\033[0m")
    print("\033[32m8. Salir\033[0m")
...
def menu():
    ...
    print('\033[32m=\033[0m'*60)
    print(' '*10 + '\033[32mSISTEMA DE GESTION: CRYPTO-ANALYTICS LAB\033[0m')
    print('\033[32m=\033[0m'*60)
    print("\033[32m1. Registrar nuevo activo (Alta)\033[0m")
    print("\033[32m2. Eliminar activo del sistema (Baja)\033[0m")
    print("\033[32m3. Modificar valoración o puntaje (Modificación)\033[0m")
    print("\033[32m4. Informe General - Visualización de los datos\033[0m")
    print("\033[32m5. Reporte - Criptomonedas con volumen superior al promedio\033[0m")
    print("\033[32m8. Salir\033[0m")
    print('\033[32m=\033[0m'*60)
...
def menu_activos_repetidos(activos_repetidos):
    ...
    for i in range(len(activos_repetidos)):
        print(f"\033[32m{i+1}.\033[0m")
        print(f"\033[32mNombre: \n\033[0m", activos_repetidos[i][0])
        print(f"\033[32mMetodología: \n\033[0m", activos_repetidos[i][4])
