...
def eliminar(lista, ticker):
    ...
    while ticker != 'fin':
        ...
        activo = busqueda_ticker(lista, ticker)
        if activo != False:
            lista.remove(activo)
        else:
...
def carga_activo(lista, nombre):
    ...
    while nombre.lower() != 'fin':
        ...
        if validacion(nombre, ticker, valor_ref, vol_act, met_op, unidades, punt_conf, lista):
            lista.append(activo)
...
def modificar_activo(lista, nombre):

    while nombre.lower() != 'fin':
        
        if busqueda_nombre(lista, nombre):

            activo_encontrado = busqueda_nombre(lista, nombre)
            menu_modificar()
            ...
            while int(opcion) != 8:
                    ...
                    if int(opcion) == 1:
                        ...
                        validacion(nuevo_nombre, lista[activo_encontrado][1], lista[activo_encontrado][2], lista[activo_encontrado][3], 1, lista[activo_encontrado][5], lista[activo_encontrado][6], lista)
                        lista[activo_encontrado][0] = nuevo_nombre

                    elif int(opcion) == 2:
                        nuevo_ticker = input("Ingrese el nuevo ticker del activo: ")
                        validacion(nuevo_ticker, lista[activo_encontrado][1], lista[activo_encontrado][2], lista[activo_encontrado][3], 1, lista[activo_encontrado][5], lista[activo_encontrado][6], lista)
                        lista[activo_encontrado][1] = nuevo_ticker

                    elif int(opcion) == 3:
                        nuevo_valor_ref = input("Ingrese el nuevo valor de referencia del activo: ")
                        validacion(lista[activo_encontrado][0], lista[activo_encontrado][1], nuevo_valor_ref, lista[activo_encontrado][3], 1, lista[activo_encontrado][5], lista[activo_encontrado][6], lista)
                        lista[activo_encontrado][2] = float(nuevo_valor_ref)

                    elif int(opcion) == 4:
                        nuevo_vol_act = input("Ingrese el nuevo volumen de actividad del activo: ")
                        validacion(lista[activo_encontrado][0], lista[activo_encontrado][1], lista[activo_encontrado][2], nuevo_vol_act, 1, lista[activo_encontrado][5], lista[activo_encontrado][6], lista)
                        lista[activo_encontrado][3] = float(nuevo_vol_act)

                    elif int(opcion) == 5:
                        metodos_validos = ['Scalping', 'Day Trading', 'Swing Trading', 'HODL'] 
                        print('1: Scalping', '2: Day Trading', '3: Swing Trading', '4: HODL')
                        nueva_met_op = input(f'Ingrese el numero de la nueva metodología de operación asignada: ')
                        validacion(lista[activo_encontrado][0], lista[activo_encontrado][1], lista[activo_encontrado][2], lista[activo_encontrado][3], nueva_met_op, lista[activo_encontrado][5], lista[activo_encontrado][6], lista)
                        lista[activo_encontrado][4] = metodos_validos[int(nueva_met_op) - 1]

                    elif int(opcion) == 6:
                        nuevas_unidades = input("Ingrese las nuevas unidades en tesorería del activo: ")
                        validacion(lista[activo_encontrado][0], lista[activo_encontrado][1], lista[activo_encontrado][2], lista[activo_encontrado][3], 1, nuevas_unidades, lista[activo_encontrado][6], lista)
                        lista[activo_encontrado][5] = float(nuevas_unidades)

                    elif int(opcion) == 7:
                        nuevo_punt_conf = input("Ingrese el nuevo puntaje de confianza del activo (1-10): ")
                        validacion(lista[activo_encontrado][0], lista[activo_encontrado][1], lista[activo_encontrado][2], lista[activo_encontrado][3], 1, lista[activo_encontrado][5], nuevo_punt_conf, lista)
                        lista[activo_encontrado][6] = int(nuevo_punt_conf)
...
def busqueda_ticker(lista, ticker):
...
def busqueda_nombre(lista, nombre):
...
def validacion(nombre, ticker, valor_ref, vol_act, met_op, unidades, punt_conf, lista):
    ...
    for i in lista:
        if metodos_validos[int(met_op)] == i[4] and nombre.upper() == i[0].upper() and ticker.upper() == i[1].upper():
...
def ordenar_matriz(lista):
    ...
    for i in range(len(lista)-1):
        ...
        for j in range(i+1, len(lista)):
            if int(lista[j][6]) > int(lista[indice_max][6]):
                indice_max = j
            elif int(lista[j][6]) == int(lista[indice_max][6]):
...
def verificacion_menu (opcion, lista):
    ...
    if opcion == 1:
        nombre = input('Ingrese el nombre oficial del activo o fin para finalizar: ')
        lista = carga_activo(lista, nombre)
        ...
        lista = modificar_activo(lista, nombre)
